...
describe("HomePage", () => {
  ...
  it("renders the landing page hero immediately", async () => {
    ...
    expect(
      screen.getAllByText(/Analyse\. Predict\./i)[0]
    ).toBeInTheDocument();

    expect(
      screen.getByText("Win.", { selector: "span" })
    ).toBeInTheDocument();
  ...
  it("renders the earliest upcoming fixture with Irish formatted date and countdown", async () => {
    ...
    expect(
      screen.getByText("29 Jan 2027, 14:15 GMT")
    ).toBeInTheDocument();
  ...
  it("navigates to login from hero login button", async () => {
    ...
    await user.click(
      screen.getAllByRole("button", { name: "Login" })[0]
    );
  ...
  it("renders footer branding", async () => {
    ...
    const footerBranding = await screen.findAllByText(
      /Analyse\. Predict\. Win\./i
    );

    expect(
      footerBranding[footerBranding.length - 1]
    ).toBeInTheDocument();
