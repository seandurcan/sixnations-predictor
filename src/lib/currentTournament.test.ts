...
import { getCurrentTournament, requireCurrentTournament } from "./currentTournament";
...
describe("current competition selection", () => {
  ...
  it("uses the explicitly selected competition when it exists", async () => {
    ...
    vi.mocked(prisma.tournament.findUnique).mockResolvedValue({ id: 7, year: 2027 } as never);
